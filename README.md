...
`http://localhost:5173`. Запити `/api`, `/media` та `/health` Vite проксює на
бекенд, тож у розробці все живе на одному походженні.

| Сторінка | Що там |
|---|---|
| `/` | каталог: фільтри, сортування, пагінація поверх живих даних |
| `/listing/{id}` | картка авто: галерея, характеристики, комплектація, продавець |

Вхід і реєстрація — вікном із шапки. **Access-токен живе лише в пам'яті
вкладки, не в `localStorage`:** те, що лежить у сховищі, читає будь-який
скрипт на сторінці, тож одна вразливість у сторонній бібліотеці віддала б
чужий доступ надовго. Після перезавантаження сесія поновлюється сама — з
httpOnly cookie, якої JavaScript не бачить узагалі.
...
Оформлення — напрямок «Збірка» з добірки макетів: холодний нейтральний фон,
IBM Plex Sans і Mono, синій акцент для дій. Бурштиновий колір зарезервований
**виключно** для лотів з торгами — оголошення з фіксованою ціною ним ніколи не
фарбується. Палітра оголошена токенами Tailwind у
[index.css](frontend/src/index.css), тож відтінок міняється в одному місці.

Довідники — кузови, пальне, коробки, марки, міста — фронтенд не тримає в себе,
а тягне з API. Оголошення приходить із сирим значенням (`fuelType: "Petrol"`),
бо саме його треба надіслати назад у фільтрі; назву для показу дає
`/api/cars/attributes`, уже перекладену сервером. Зводить їх докупи
[useAttributeLabels](frontend/src/api/useAttributeLabels.ts).

Галерея на картці авто бере мініатюри окремим файлом, а не масштабує великі
зображення — інакше сторінка вантажила б десяток повнорозмірних знімків заради
смужки внизу. Лічильник переглядів росте лише на чужих відкриттях: свої
перегляди автор накрутити не може.
