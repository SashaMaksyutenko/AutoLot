...
public sealed class Listing : AuditableEntity
{
    ...
    // ── Угода ────────────────────────────────────────────────────────
    //
    // Окремої сутності «угода» немає навмисно: на одне оголошення припадає
    // щонайбільше одна продажа, і таблиця з одним рядком на оголошення лише
    // додала б з'єднання до кожного запиту. Якщо колись знадобиться історія
    // (продали, повернули, продали знову) — тоді й буде привід її завести.

    /// <summary>
    /// Кому продали. Порожнє навіть у проданого лота, і це нормальний
    /// випадок: авто могли продати сусідові, а оголошення просто закрити.
    ...
    /// <summary>
    /// Коли продали. Окремо від <see cref="Status"/>, бо статус відповідає
    /// лише на «чи продано», а відгук про угоду треба буде десь прив'язати
    /// в часі.
    /// </summary>
    ...
    /// <param name="now">Час угоди.</param>
    /// <param name="buyerId">
    /// Кому продали, якщо покупець є на майданчику. Порожнє означає «продано
    /// поза AutoLot» — теж чесна відповідь, і саме тому не обов'язкове.
    /// </param>
    public void MarkSold(DateTimeOffset now, long? buyerId = null)
    {
        ...
        // оголошення; чи покупець узагалі існує й чи він писав продавцю —
